...
Vector2d controller::Trunk_gravity_compensation(StateModel_* state_model, int F_R)
{
    
// double F_front = 28 * 9.81 * 16 / 33 * 0.5;
// double F_rear = 28 * 9.81 * 17 / 33 * 0.5;
    double Ff = F_front;
    double Fr = F_rear;
    Vector2d fRW;
    if (F_R == 1)
    {
        fRW[0] = Ff * cos(state_model->posRW[1]);
        fRW[1] = Ff * sin(state_model->posRW[1]);
    }
    else
    {
        fRW[0] = Fr * cos(state_model->posRW[1]);
        fRW[1] = Fr * sin(state_model->posRW[1]);
    }
    return fRW;
}
